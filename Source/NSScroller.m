...
{
   //NSPoint point = [[[theEvent touchesForView: self] anyObject] locationInWindow];
   printf("NSScroller: touchesBegan, view touch count = %i, touch set count = %i, first touch = %i, first touch location x = %f, y = %f\n", _touchCount, [touches count], _first_touch_id, _first_touch_location_in_view.x, _first_touch_location_in_view.y);
}

- (void) touchesMoved: (NSSet *) touches withEvent: (NSEvent *) theEvent
{
   //NSPoint point = [[[theEvent touchesForView: self] anyObject] locationInWindow];
   printf("NSScroller: touchesMoved, view touch count = %i, touch set count = %i, first touch = %i, first touch location x = %f, y = %f\n", _touchCount, [touches count], _first_touch_id, _first_touch_location_in_view.x, _first_touch_location_in_view.y);
}

- (void) touchesEnded: (NSSet *) touches withEvent: (NSEvent *) theEvent
{
   //NSPoint point = [[[theEvent touchesForView: self] anyObject] locationInWindow];
   printf("NSScroller: touchesEnded, view touch count = %i, touch set count = %i, first touch = %i, first touch location x = %f, y = %f\n", _touchCount, [touches count], _first_touch_id, _first_touch_location_in_view.x, _first_touch_location_in_view.y);
}
...
{ 
  printf("NSScroller mouseDown, view mouse count = %i, first mouse = %i\n", _mouseCount, _first_mouse_id);
  ...
  printf("event type = %i, hitPart = %i\n", [theEvent type], _hitPart);
  ...
  switch (_hitPart)
    {
      ...
      case NSScrollerDecrementPage:
        //printf("NSScroller calling trackScrollButtons...\n");
...
- (void) trackKnob: (NSEvent*)theEvent
{
  unsigned int eventMask = NSLeftMouseDownMask | NSLeftMouseUpMask
			  | NSLeftMouseDraggedMask | NSFlagsChangedMask | SNTouchedMask 
  ...
  if (_isHorizontal)
    {
      ...
      if (eventType == NSLeftMouseDown)
        _trackKnob_offset = lastPosition - point.x;
    }
  else
    {
      ...
      if (eventType == NSLeftMouseDown)
        _trackKnob_offset = lastPosition - point.y;
    }

  //initialOffset = offset; /* Save the initial offset value */
  initialOffset = _trackKnob_offset; /* Save the initial offset value */
  ...
  //  {
	   ...
	   eventType = [theEvent type];
           //printf("NSScroller grabs an event! #1\n"); //printed continuously when dragging

           while (theEvent && eventType == SNTouched) {
              // We trapped a touch event, re-deliver it
              printf("NSScroller delivering touch event... #1\n");
              [NSApp sendEvent: theEvent];
              if ([theEvent touchCount] > 1) { // We have a multi-touch event, cancel tracking
                 printf("NSScroller loop ending... #1\n");
                 return;
              }
              theEvent = [NSApp nextEventMatchingMask: eventMask
	...
	*/
       ...
       newPosition = point.x + _trackKnob_offset;
       else
      //   newPosition = point.y + offset;
       newPosition = point.y + _trackKnob_offset;
       ...
       if (newPosition != lastPosition)
           ...
           if (flags & NSAlternateKeyMask)
	     {
	       ...
	       diff = diff * 3 / 4;
	       //offset -= diff;
               _trackKnob_offset -= diff;
	       newPosition -= diff;
	     }
	   else //== Ok, we are no longer doing slow scrolling, lets go back to our original offset. ==//
	     {
	       //offset = initialOffset;
               _trackKnob_offset = initialOffset;
	     }
	...
	*/
       ...
       theEvent = [NSApp nextEventMatchingMask: eventMask
                     ...
                     dequeue: YES];
       eventType = [theEvent type];
       //printf("NSScroller grabs an event! #2\n"); //printed continuously when dragging

       while (eventType == SNTouched) {
          // We trapped a touch event, re-deliver it
          printf("NSScroller delivering touch event... #2\n");
          [NSApp sendEvent: theEvent];
          if ([theEvent touchCount] > 1) { // We have a multi-touch event, cancel tracking
             printf("NSScroller loop ending... #2\n");
             return;
          }
          theEvent = [NSApp nextEventMatchingMask: eventMask
...
- (void) trackScrollButtons: (NSEvent*)theEvent
{
  ...
  //printf("NSScroller trackScrollButtons\n");
  [self lockFocus];
  ...
  if (theCell)
    {
      BOOL pointIsInCell = [self mouse: [self firstMouseLocationInView] inRect: rect];
       ...
       */
      BOOL done = [theCell trackMouse: theEvent
		   inRect: rect
		   ofView: self
	     untilMouseUp: YES];
