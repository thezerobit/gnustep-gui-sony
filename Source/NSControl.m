...
{
  //printf("NSControl sendAction:to:\n");
...
{
  printf("NSControl mouseDown, view mouse count = %i, first mouse = %i\n", _mouseCount, _first_mouse_id);

  // If multi-touch is not enabled, ignore all but first touch
  // This also rejects periodic events generated by other touches
  //if (!_multipleTouchEnabled && [theEvent mouse] != _first_mouse_id)
  //   return;
  ...
  //while (1)
  //  {
      //printf("NSControl while loop.. event type = %i, control = %s\n", [e type], [[self description] cString]);
      NSPoint location = [self convertPoint: [e locationInWindow] 
			       fromView: nil];

      // ask the cell to track the mouse only,
      // if the mouse is within the cell
      if ([self mouse: location inRect: _bounds] || [[_cell class] prefersTrackingUntilMouseUp])
	{
	  BOOL done;

	  [_cell setHighlighted: YES];
	  [self setNeedsDisplay: YES];
          //printf("calling cell trackMouse...\n");
	  ...
	  //if (done || _gotMultiTouch)
          if (done)
	    {
              //printf("break 1.. mouseUp within cell\n");
	      //break;
	      [_cell setHighlighted: NO];
	      [self setNeedsDisplay: YES];
	    }
      ...
      while (1) {
         if ([e type] == SNTouched) {
            ...
            if ([e touchCount] > 1) { // We have a multi-touch event, cancel tracking
               //_gotMultiTouch = YES;
               //printf("NSControl loop ending... \n");
               //break;
            }
         //} else if (([e type] == NSLeftMouseDown | [e type] == NSLeftMouseUp | [e type] == NSLeftMouseDragged) &&
         //          ([[[e window] contentView] hitTest: [e locationInWindow]] != self)) {
         } else if (([e type] == NSLeftMouseDown | [e type] == NSLeftMouseUp | [e type] == NSLeftMouseDragged) &&
                     [[e window] viewForEvent: e] != self) {
            //printf("Button #%s got somebody else's mouse event... 1\n", [_contents cString]);
            _crState = 1;
            [NSApp sendEvent: e];
            return;
         } else {
      ...
      ***/
      ...
      //if (_gotMultiTouch || [e type] == NSLeftMouseUp)
      if ([e type] == NSLeftMouseUp)
        {
          //printf("NSControl NSLeftMouseUp\n");
          /**
          if ([e endOfSequence]) {
             printf("NSControl cleaning mouse-view dictionary...\n");
             [[e window] cleanMouseViewDictionary];
          }
          **/
          //printf("break 2.. mouseUp or multi-touch outside bounds\n");
	  //break;
        }
    //}
...
{
  printf("NSControl %s: touchesBegan, touch set count = %i\n", [[self title] cString], [touches count]);
}
...
- (void) handleSingleTouch: (NSEvent *) theEvent
{
   ...
   SNTouch *theTouch = [[theEvent touchesForView: self] anyObject];
   NSPoint location = [self convertPoint: [theTouch locationInWindow] 
			        fromView: nil];

   // ask the cell to track the touch only,
   // if the touch is within the cell
   if ([self mouse: location inRect: _bounds])
   {
      ...
      [_cell setHighlighted: YES];
      [self setNeedsDisplay: YES];
      printf("calling cell trackTouch...\n");
      done = [_cell trackTouch: theEvent
			inRect: _bounds	
			ofView: self	
               untilTouchEnded: [[_cell class] prefersTrackingUntilMouseUp]];
...
{
  printf("NSControl %s: touchesBeganWithEvent, touch count = %i\n", [[self title] cString], [theEvent touchCount]);

  // If not enabled ignore touches
  if (![self isEnabled])
    {  
  ...
  NSSet *touchSet = [theEvent touchesForView: self];

  if ([touchSet count] == 1)
  {
     [self handleSingleTouch: theEvent];
  }
  else //zero or more than one touch in view
  {
  }
...
{
   printf("NSControl %s: touchesMovedWithEvent, touch count = %i\n", [[self title] cString], [theEvent touchCount]);
...
{
   printf("NSControl %s: touchesEndedWithEvent, touch count = %i\n", [[self title] cString], [theEvent touchCount]);
  ...
  {
     printf("NSControl cleaning touch-view dictionary...\n");
...
/**<p>Returns whether multiple clicks are ignored.</p>
...
/** <p>Sets whether multiple clicks are ignored.</p>
